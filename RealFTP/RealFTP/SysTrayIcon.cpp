...
LRESULT CSysTrayWnd::OnSysTrayMsg(WPARAM wParam, LPARAM lParam)
{
	...
	const long TRAY_CLICK = 512;
	const long BUBLE_CLICK = 1029;
    ...
    if (bIconCreated == FALSE)
    {
        ...
        if (pnidIconInfo != NULL)
        {
            ...
            if (hIcon != NULL)
            {
				...
				char tip[512];
				wtc(tip, pnidIconInfo->szTip);
				
				::strcpy( tip , szTip); // Copy the string
...
BOOL CSysTrayIcon::ShowBalloon(UINT  nIconID, PTSTR szBalloonTitle, PTSTR szBalloonMsg, DWORD dwIcon, UINT nTimeOut)
{
	...
	::wcscpy(pnidIconInfo->szInfoTitle, szBalloonTitle);

	// Set balloon message
	::wcscpy(pnidIconInfo->szInfo, szBalloonMsg);
...
{
    LPCTSTR pstrOwnerClass = ::AfxRegisterWndClass(0); // PCSTR
...
BOOL CSysTrayIcon::ChangeIconTip(UINT nIconID, CString strTip)
{
    ...
    if (pnidIconInfo != NULL)
    {
        ::wcscpy(pnidIconInfo->szTip, szTip); // Copy the new string containing the tip
...
*/
void CSysTrayIcon::wtc(char* to, wchar_t* from)
{
	size_t n = 999999;
