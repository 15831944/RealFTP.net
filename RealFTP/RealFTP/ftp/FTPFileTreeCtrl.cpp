...
bool CFTPFileTreeCtrl::DisplayTree(CFTPClient* pFTP, const CString& cszDisplayName, const CString& cszRootDir, bool fFiles)
{
   ...
   m_RootInfo.m_cszDisplayName = cszDisplayName + _T(" ");

   m_RootInfo.m_htiRoot = AddItem(TVI_ROOT, cszDisplayName + _T(" ") + cszRootDir, true);
...
void CFTPFileTreeCtrl::DisplayPath(HTREEITEM hParent)
{
   ...
   TSpFTPFileStatusVector vFileList;
   ...
   for( TSpFTPFileStatusVector::iterator it = vFileList.begin(); it!=vFileList.end(); it++ )
   {
      if( (*it)->IsCwdPossible() )
      {
         if( !(*it)->IsDot() )
...
void CFTPFileTreeCtrl::GetDirectoryListing(HTREEITEM hItem, TSpFTPFileStatusVector& vFileList) const
{
   ASSERT( m_pFTP );
...
CString CFTPFileTreeCtrl::GetFullPath(HTREEITEM hItem) const
{
   ...
   if( cszFullPath.GetLength() > m_RootInfo.m_cszDisplayName.GetLength() )
      cszFullPath = cszFullPath.Mid(m_RootInfo.m_cszDisplayName.GetLength());
